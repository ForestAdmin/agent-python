...
class SchemaEmitter:
    ...
    @staticmethod
    def serialize(collections: List[ForestServerCollection], meta: AgentMeta) -> Dict[str, Any]:
        serialized_collections: List[Dict[str, Any]] = []
        for collection in collections:
            actions = collection.pop("actions")
            ...
            serialized_collections.append(
                {
                    ...
                    "relationships": {"actions": {"data": actions}, "segments": {"data": segments}},
                }
            )
        return {"data": serialized_collections, "included": [], "meta": meta}
    ...
    async def get_serialized_schema(
        ...
        if not options["is_production"]:
            schema = SchemaEmitter.generate(options["prefix"], datasource)
        return schema
    ...
    def generate(
        ...
        for collection in datasource.collections:
            collection_schema.append(SchemaCollectionGenerator.build(prefix, collection))
