...
class DjangoCollectionFactory:
    ...
    @staticmethod
    def _build_many_to_one(relation: Union[OneToOneRel, ForeignKey, ManyToOneRel]) -> Optional[ManyToOne]:
        if isinstance(relation, OneToOneRel) or isinstance(relation, ManyToOneRel):
            foreign_key = relation.field.column
        elif isinstance(relation, ForeignKey):
    ...
    @staticmethod
    def _build_one_to_one(relation: OneToOneField) -> OneToOne:
        return {
            "foreign_collection": relation.target_field.model.__name__,
            "origin_key": relation.column,
            "origin_key_target": relation.target_field.name,
    ...
    def build(model: Model) -> CollectionSchema:
        ...
        for field in model._meta.get_fields():
            if not field.is_relation:
                fields[field.name] = FieldFactory.build(field)  # type: ignore
            else:
