...
class TestWriteSimpleCreation(TestCase):
    @classmethod
    ...
    def test_should_do_nothing(self):
        with patch.object(self.collection_book, "create", new_callable=AsyncMock) as mock_book_create:
            self.loop.run_until_complete(
                self.collection_book_decorated.create(self.mocked_caller, [{"name": "a name"}])
            )

            mock_book_create.assert_any_await(self.mocked_caller, [{"name": "a name"}])
    ...
    def test_should_work_with_empty_handler(self):
        ...
        with patch.object(self.collection_book, "create", new_callable=AsyncMock) as mock_book_create:
            self.loop.run_until_complete(
                self.collection_book_decorated.create(self.mocked_caller, [{"name": "a name", "age": "some age"}])
            )

            mock_book_create.assert_any_await(self.mocked_caller, [{"age": "some age"}])
    ...
    def test_should_work_writing_on_the_same_field_in_handler(self):
        ...
        with patch.object(self.collection_book, "create", new_callable=AsyncMock) as mock_book_create:
            ...
            mock_book_create.assert_any_await(self.mocked_caller, [{"name": "another name"}])
        handler.assert_called_with("a name", ANY)
        ...
        with patch.object(self.collection_book, "create", new_callable=AsyncMock) as mock_book_create:
            self.loop.run_until_complete(
                self.collection_book_decorated.create(
                    self.mocked_caller, [{"name": "name", "price": "price", "age": "age"}]
                )
            )

            mock_book_create.assert_any_await(
                self.mocked_caller, [{"name": "name", "price": "new price", "age": "new age"}]
            )
    ...
    def test_should_work_doing_nested_rewritting(self):
        ...
        with patch.object(self.collection_book, "create", new_callable=AsyncMock) as mock_book_create:
            ...
            mock_book_create.assert_any_await(self.mocked_caller, [{"price": "some price"}])
    ...
    def test_should_throw_when_two_handlers_request_conflicts_updates(self):
        ...
        self.assertRaisesRegex(
            ...
            self.loop.run_until_complete,
            self.collection_book_decorated.create(self.mocked_caller, [{"name": "a name", "age": "an age"}]),
        )
    ...
    def test_should_raise_when_handler_return_unexpected_type(self):
        ...
        self.assertRaisesRegex(
            ...
            self.loop.run_until_complete,
            self.collection_book_decorated.create(self.mocked_caller, [{"age": "10"}]),
        )
