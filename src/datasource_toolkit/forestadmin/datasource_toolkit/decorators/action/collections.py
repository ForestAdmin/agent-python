...
from forestadmin.datasource_toolkit.decorators.action.fields import (
    BaseDynamicField,
    DynamicFormElement,
    FormElement,
    FormElementFactory,
    LayoutDynamicElement,
    LayoutDynamicField,
)
...
class ActionCollectionDecorator(CollectionDecorator):
    ...
    async def get_form(
        ...
        form_fields: List[DynamicFormElement] = cast(
            List[DynamicFormElement], [field for field in action.get("form", [])]
        )
    ...
    async def _build_form_values(
        ...
        for field in fields:
            if not isinstance(field, LayoutDynamicElement):
    ...
    async def _build_fields(
        ...
        fields: List[DynamicFormElement],
        form_values: RecordsDataAlias,
